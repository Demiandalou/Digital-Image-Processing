...
def visulize(mask):
    ...
    white = np.zeros_like(I)
    white[:,:,0].fill(255)
    white[:,:,1].fill(255)
    white[:,:,2].fill(255)
    np.copyto(res, white, where=mask)
...
# bg=parse_data('bg_points.txt')
# fg=parse_data('fg_points.txt')

I=cv2.imread("../family.jpg")
bg=parse_data('bg_points2.txt')
fg=parse_data('fg_points2.txt')
